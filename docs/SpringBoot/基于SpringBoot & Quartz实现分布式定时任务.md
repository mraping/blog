## 问题引入
SpringBoot是已经集成了定时任务的，使用@Scheduled可以创建定时任务，但是有其局限性，在遇到一些复杂的逻辑则需要引入quartz定时任务框架
使用定时任务框架quartz可以完成分布式单节点定时任务持久化，将任务持久化到数据库，能够更好的预防任务丢失。
## 与SpringBoot的整合
### 配置pom依赖
```
<!--quartz依赖-->
...
public class PushTimer extends QuartzJobBean {
    ...
    protected void executeInternal(JobExecutionContext jobExecutionContext) throws JobExecutionException {
        logger.info("分布式节点quartz-cluster-node-first，获取当天推送任务，任务时间：{}",new Date());
    }
}
```
...
通过调用buildPushTimer来开启定时任务，这里采用在项目初始化的时候来构建定时任务
```
@Component
public class QuartzApplicationRunner implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) throws Exception {
        quartzService.buildGetAllPushTimer();
...
## 分布式任务
通过复制同样的项目，更改port及application name来测试分布式任务调度，可以发现当一个节点挂掉时，另一个节点会自动接管任务。
需要注意的是：分布式定时任务触发时间只能有一次
